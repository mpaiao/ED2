...
#==========================================================================================#
#==========================================================================================#
#      Function that is similar to cumsum, except that is does not let the values exceed   #
# or fall below a given threshold.  This is useful for integrating thinks like cumulative  #
# water deficit.                                                                           #
#------------------------------------------------------------------------------------------#
thresh.cumsum <<- function(x,xthresh,thmax=TRUE,na.rm=FALSE){
   #----- Get the vector size. ------------------------------------------------------------#
   nx = length(x)
   #---------------------------------------------------------------------------------------#

   #----- Initialise the vector with NA.  We will fill them if there is anything to fill. -#
   ans = NA_real_ * x
   #---------------------------------------------------------------------------------------#


   #----- In case na.rm is TRUE, switch missing values with x. ----------------------------#
   if (na.rm) x[is.na(x) | is.nan(x)] = 0.
   #---------------------------------------------------------------------------------------#

   #---------------------------------------------------------------------------------------#
   #      Skip calculation if all values are missing.                                      #
   #---------------------------------------------------------------------------------------#
   if (all(! (is.na(x) | is.nan(x)))){
      ...
      if (thmax){
         ...
         ans[1] = min(c(xthresh,x[1]))
         for (i in sequence(nx)[-1]){
            ans[i] = min(xthresh,ans[i-1]+x[i])
