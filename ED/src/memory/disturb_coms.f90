...
module disturb_coms
   ...
   !    soil depth is 1 m, so deeper soils must be much drier to allow fires to happen.    !
   !    Kept as an option for legacy, but strongly discouraged.                            !
   ! 2. (ED-2.2 default) Fire will be triggered with enough biomass and the total soil     !
   !    water at the top 50cm falls below a (relative) threshold.                          !
   ! 3. (Beta) The EMBERFire model (Longo et al. in prep).  Empirical approach loosely     !
   !    based on option 2, with the following differences:                                 !
   !    - Fuel.  Fast/structural carbon + understory (h<2m) biomass.                       !
   !    - Fire ignition probably based on Nesterov index, similar to to SPITFIRE (Thonicke !
   !      et al. 2010)                                                                     !
   !    - Mortality.  Simple sigmoidal function of bark thickness. (no account for fire    !
   !      intensity).                                                                      !
   !    - Combustion. A fraction of fuels is volatilised during the disturbance, and the   !
   !      remainder is accumulated as litter.                                              !
   ! 4. (In development) FIRESTARTER model (Longo et al. in prep).  This is a more         !
   !    process-based fire model that builds on fire density, ignition and                 !
   !    termination from HESFIRE (LePage et al. 2015), and fire damage and spread rates    !
   !    based on SPITFIRE (Thonicke et al. 2010). This module is still under development   !
   !    and may change.  Feel free to try it, but mind that you may need to optimise       !
   !    parameters and likely do some debugging.  Also, please reach out if intending to   !
   !    use it in new projects or publications.                                            !
   !---------------------------------------------------------------------------------------!
   ...
   real(kind=4) :: fi_hdi_lwr    ! Upper bound for HDI effect on ignition      [       ---]
   real(kind=4) :: fi_hdi_upr    ! Upper bound for HDI effect on ignition      [       ---]
   real(kind=4) :: fi_hdi_dti    ! (*) 1. / ( fi_hdi_upr - fi_hdi_lwr )        [       ---]
  ...
  !----- Termination parameters. ---------------------------------------------------------!
   ...
   real(kind=4) :: ft_hdi_lwr    ! Lower bound for HDI effect on termination   [       ---]
   real(kind=4) :: ft_hdi_upr    ! Upper bound for HDI effect on termination   [       ---]
   real(kind=4) :: ft_hdi_dti    ! (*) 1. / ( ft_hdi_upr - ft_hdi_lwr )        [       ---]
   real(kind=4) :: ft_hdi_exp    ! Exponent for HDI effect on termination      [       ---]
   real(kind=4) :: ft_fdi_upr    ! Upper bound for FDI effect on termination   [       ---]
   real(kind=4) :: ft_fdi_exp    ! Exponent for FDI effect on termination      [       ---]
   !=======================================================================================!
   !=======================================================================================!
