...
module ed_init_history
  contains
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This subroutine will set a full history start for the simulation.  In a full      !
   ! history start (runtype = 'HISTORY'), we assumes that you are continuing with the      !
   ! exact same configuration as a given model simulation that wrote the file in which you !
   ! are using.  In this case, each node starts with a list of polygons, and searches the  !
   ! HDF history file for these polygons.  It expects to find at least one polygon in the  !
   ! file within 250 meters of proximity (although it should be exactly at the same        !
   ! place).  If it does not find this it stops.                                           !
   ...
   ! polygon, initializing the model to the exact same state as the end of the previous    !
   ! run.  A search based restart does not expect to find exact matches, and may not use   !
   ...
   subroutine resume_from_history()
      ...
      ! ED2 error reporting will detect it.  If something that can't be found is missing,  !
      ! the following call can be bypassed.  Note that automatic error reporting is turned !
      ! back on in the end.                                                                !
      !------------------------------------------------------------------------------------!
      ...
      gridloop: do ngr=1,ngrids
         ...
         !---------------------------------------------------------------------------------!
         !      In case this is the first grid, we read global dimensions and vectors.     !
         !---------------------------------------------------------------------------------!
         select case (ngr)
         case (1)
         ...
         ! After the match, we must walk through the data from that polygon and            !
         ! initialize.  We check the distance between the expected coordinates and the     !
         ! retrieved ones, and they ought to be less than 250 metres apart, otherwise we   !
         ! can't use the polygon.                                                          !
         !---------------------------------------------------------------------------------!
                  ...
                  if (sipa_n(si_index) > 0) then
                     ...
                     patchloop: do ipa = 1,csite%npatches
                        ...
                        if (paco_n(pa_index) > 0) then
                           ...
                           !---------------------------------------------------------------!
                           !     Get all necessary site variables associated with this     !
                           ! index for the current patch.                                  !
                           !---------------------------------------------------------------!
   ...
   subroutine fill_history_grid_p11(cgrid,ipy,py_index)
      ...
      ! according to the dimensionality.  If you are adding a new variable, make sure that !
      ! it is placed together with variables of the same dimensions.                       !
      !                                                                                    !
   ...
   subroutine fill_history_grid_p11dmean(cgrid,ipy,py_index)
      ...
      if (writing_long) then
         ...
         call hdf_getslab_r(cgrid%dmean_nppbark        (ipy:ipy)                           &
                        ,'DMEAN_NPPBARK_PY          ',dsetrank,iparallel,.false.,foundvar)
         ...
         call hdf_getslab_r(cgrid%dmean_stem_resp      (ipy:ipy)                           &
                        ,'DMEAN_STEM_RESP_PY        ',dsetrank,iparallel,.false.,foundvar)
   ...
   subroutine fill_history_grid_p11mmean(cgrid,ipy,py_index)
      ...
      if (writing_eorq) then
         ...
         call hdf_getslab_r(cgrid%mmean_stem_resp      (ipy:ipy)                           &
                        ,'MMEAN_STEM_RESP_PY        ',dsetrank,iparallel,.false.,foundvar)
         ...
         call hdf_getslab_r(cgrid%mmean_nppbark        (ipy:ipy)                           &
                        ,'MMEAN_NPPBARK_PY          ',dsetrank,iparallel,.false.,foundvar)
   ...
   subroutine fill_history_grid_m11(cgrid,ipy,py_index)
      ...
      if (writing_dcyc) then
         ...
         call hdf_getslab_r(cgrid%qmean_stem_resp      (:,ipy)                             &
                        ,'QMEAN_STEM_RESP_PY       ',dsetrank,iparallel,.false.,foundvar)
   ...
   subroutine fill_history_polygon(cpoly,pysi_index,nsites_global,nsites_now,is_burnt)
      ...
      !------------------------------------------------------------------------------------!
      !       The _SI extension has been dropped as LSL and soil colour are no longer      !
      ! polygon variables.  In case the history is from an old file, they may still exist, !
      ! so we try them first and if it doesn't work we try the one without extension.      !
      !------------------------------------------------------------------------------------!
      ...
      ! MLO. Variable NAT_DIST_TYPE was deleted because now burnt patches are              !
      !      distinguished from tree fall patches.   Thus, if NAT_DIST_TYPE exists in the  !
      !      file, then it must be and old history file.                                   !
      !------------------------------------------------------------------------------------!
      ...
      ! variable.  In case the history is from an old file, they may still exist, so we    !
      ! try it first and if it doesn't work we try the one without extension.              !
      !------------------------------------------------------------------------------------!
      ...
      ! number of land use types has changed, we must check whether this history file is   !
      ! old or new.                                                                        !
      !------------------------------------------------------------------------------------!
      if (old_histo) then
         !---------------------------------------------------------------------------------!
         ...
         ! polygon variable.  In case the history is from an old file, they may still      !
         ! exist, so we try it first and if it doesn't work we try the one without         !
         ! extension.                                                                      !
         !---------------------------------------------------------------------------------!
         ...
         do isi=1,cpoly%nsites
            ...
            !------------------------------------------------------------------------------!
            !     Find the indices for disturbance transitions based on the last fire      !
            ! regime and whether it is a forest plantation of an explored stand.  The      !
            ! translation won't be perfect, because we cannot retrieve all information     !
            ! needed.                                                                      !
            !------------------------------------------------------------------------------!
   ...
   subroutine fill_history_site(csite,sipa_index,npatches_global,is_burnt)
      ...
      !------------------------------------------------------------------------------------!
      !------------------------------------------------------------------------------------!
      !      Check whether this is an old history file (back when there were only 3        !
      ! types).  If this is the case, we must translate the indices to the new disturbance !
      ! table.  This is not going to be perfect because the number of disturbance types    !
      ! increased,  but this should be rarely needed, as it is not a good idea to switch   !
      ! versions then continue to run the model using binary-history.                      !
      !------------------------------------------------------------------------------------!
      ...
      ! integrator was entirely converted to double precision, we reverted back to single  !
      ! and converted everything just for the Runge-Kutta.  Of course, nothing in life is  !
      ! certain but death and taxes, so we kept the recipe on how to read a double         !
      ! precision if we ever change our minds again.                                       !
      !                                                                                    !
   ...
   subroutine fill_history_patch(cpatch,paco_index,ncohorts_global)
      ...
      !------------------------------------------------------------------------------------!
      !     If this patch is empty, we don't need to do anything (actually the less we do  !
      ! the better, we don't want to add segmentation violations in our beloved model!).   !
      !------------------------------------------------------------------------------------!
      ...
      call hdf_getslab_r(cpatch%today_stem_resp                                            &
                        ,'TODAY_STEM_RESP           ',dsetrank,iparallel,.true. ,foundvar)
      ...
      call hdf_getslab_r(cpatch%today_nppbark                                              &
                        ,'TODAY_NPPBARK             ',dsetrank,iparallel,.true. ,foundvar)
      ...
      call hdf_getslab_r(cpatch%stem_respiration                                           &
                        ,'STEM_RESPIRATION          ',dsetrank,iparallel,.true. ,foundvar)
      ...
      if (writing_long) then
         ...
         call hdf_getslab_r(cpatch%dmean_nppbark                                           &
                        ,'DMEAN_NPPBARK_CO          ',dsetrank,iparallel,.false.,foundvar)
         ...
         call hdf_getslab_r(cpatch%dmean_stem_resp                                         &
                        ,'DMEAN_STEM_RESP_CO        ',dsetrank,iparallel,.false.,foundvar)
      ...
      if (writing_eorq) then
         ...
         call hdf_getslab_r(cpatch%mmean_stem_resp                                         &
                        ,'MMEAN_STEM_RESP_CO        ',dsetrank,iparallel,.false.,foundvar)
         ...
         call hdf_getslab_r(cpatch%mmean_nppbark                                           &
                        ,'MMEAN_NPPBARK_CO          ',dsetrank,iparallel,.false.,foundvar)
      ...
      if (writing_dcyc) then
         ...
         call hdf_getslab_r(cpatch%qmean_stem_resp                                         &
                        ,'QMEAN_STEM_RESP_CO        ',dsetrank,iparallel,.false.,foundvar)
   ...
   !              If a variable is missing and required is set to true, then the model     !
   !              will crash.  If it is missing but required is false, then we initialise  !
   !              the variable with zeroes and print a big banner to the standard output   !
   !              to warn the user.                                                        !
   ...
   subroutine hdf_getslab_r(buff,varn,dsetrank,iparallel,required,foundvar)
      ...
      if ((.not. foundvar) .and. required ) then
         ...
         write (unit=*,fmt='(a)') ' NOT ABSOUTELY NECESSARY TO RESTART PROGNOSTIC STATE'
         write (unit=*,fmt='(a)') '-------------------------------------------------------'
         write (unit=*,fmt='(a)') ''
         end if
