module ed_init
         ...
         polyloop: do ipy=1,cgrid%npolygons
            ...
            siteloop: do itext=1,maxsite
               if (work_v(ifm)%soilfrac(itext,ipy) > min_site_area) then
                  ...
                  !------------------------------------------------------------------------!
                  !     Initialise area, using the soil texture area scaled by the total   !
                  ! being used.                                                            !
                  !------------------------------------------------------------------------!
                  ...
                  !------------------------------------------------------------------------!
                  !       Set soil moisture decay function, based on second layer's K      !
                  ! value.  We use the second layer instead of the top in case top is      !
                  ! organic/peat.                                                          !
                  !------------------------------------------------------------------------!
            ...
            !----- This should not happen in this sub-routine... --------------------------!
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This subroutine fills the lsl, soil colour, and soil texture based on the         !
   ! defaults.  In case isoildepthflg was zero, then the layer_index matrix was filled     !
   ...
   !=======================================================================================!
   !=======================================================================================!
   !    Several Procedures requiring ASCII reads follow.  If this is a parallel run, then  !
   ! the nodes must queue.  Since we will access sequential format files, each node needs  !
   ! to wait its turn to access it... MPI_File commands won't work with ASCII files, so    !
   ! that's a bottleneck here. If the run is serial mynum=nnodetot, so I don't need to     !
   ! wait.                                                                                 !
   !---------------------------------------------------------------------------------------!
   subroutine load_ecosystem_state()
      ...
      if (mynum == 1) then
         write(unit=*,fmt='(a)') ' + Doing sequential initialization over nodes.'
      end if
      ...
      if (mynum < nnodetot) then
         call MPI_Send(ping,1,MPI_INTEGER,sendnum,100,MPI_COMM_WORLD,ierr)
      end if
#endif
      !------------------------------------------------------------------------------------!
      ...
      case (-8,-1,0)
         ...
         case (1)
            ...
            if (mynum /= 1) then
               write(unit=*,fmt='(a)')                                                     &
                   ' + Doing near-bare-ground big-leaf initialization...'
            end if
            do igr=1,ngrids
...
#if defined(RAMS_MPI)
      if (mynum < nnodetot) then
         call MPI_Send(ping,1,MPI_INTEGER,sendnum,101,MPI_COMM_WORLD,ierr)
      end if
#endif
      !------------------------------------------------------------------------------------!
...
#if defined(RAMS_MPI)
      if (mynum < nnodetot ) then
         call MPI_Send(ping,1,MPI_INTEGER,sendnum,102,MPI_COMM_WORLD,ierr)
      end if
#endif
      !------------------------------------------------------------------------------------!
...
#if defined(RAMS_MPI)
      if (mynum < nnodetot ) then
         call MPI_Send(ping,1,MPI_INTEGER,sendnum,103,MPI_COMM_WORLD,ierr)
      end if
#endif
      !------------------------------------------------------------------------------------!
   ...
   !=======================================================================================!
   !=======================================================================================!
   !     This subroutine defines the variables related to the soil layers, and also        !
   ! initialises some RK4 variables that depend on the soil grid.                          !
   !---------------------------------------------------------------------------------------!
   ...
   subroutine read_obstime()
      ...
      if (.not.((outfast .eq. frqfast) .or. (outfast .eq. 0))) then
         write (unit=*,fmt='(a,1x,f7.0,1x,a)') &
            'OUTFAST must be 0 or equal to FRQFAST for observation time output. (Yours is '&
            ,outfast,').'
   ...
   subroutine remove_obstime(time_idx)
      ...
      if (time_idx < obstime_list_len) then
          ...
          ! print error message
          write (unit=*,fmt='(i4,a,i4)')                                                   &
                   time_idx, 'is out of the dimension of obstime_list, ', obstime_list_len
          call fatal_error('Out of obstime_list','remove_obstime','ed_init.F90')
   ...
   subroutine is_obstime(year,month,date,time,observation_time,time_idx)
      ...
      do while ((.not. observation_time) .and. (time_idx <= obstime_list_len))
           time_idx = time_idx + 1
           observation_time = obstime_list(time_idx)%year  == year   .and.                 &
                              obstime_list(time_idx)%month == month  .and.                 &
                              obstime_list(time_idx)%date  == date   .and.                 &
                              obstime_list(time_idx)%time  == time
      end do
      return
